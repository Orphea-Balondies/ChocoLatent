...
def parse_args():
    ...
    parser.add_argument('--eps', nargs='*', type=float, default=[0.07])
...
def pgd(X, model, image_index, eps=0.1, step_size=0.2, iters=200, clamp_min=-1, clamp_max=1, mask=None, Lpipsvalue=10):
    ...
    for i in pbar:
        ...
        gap2O = (model(X_adv).latent_dist.mean-model(X).latent_dist.mean).norm()
        gap = gap2black
        #appro = (X-X_adv).norm()
        loss = gap + ratio2*appro
