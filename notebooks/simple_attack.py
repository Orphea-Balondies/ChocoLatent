...
def parse_args():
    ...
    parser.add_argument('--dir', type=str, default='/home/zhangjianwei/photoguard/init_images/mix-27/', help='The main directory path of images')
    ...
    parser.add_argument('--out_dir', type=str, default='/home/zhangjianwei/photoguard/output/test', help='select the output directory')
    parser.add_argument('--mode', type=str, default='protect', choices=['all', 'protect', 'infer'])
...
def main():
    ...
    if not args.out_dir:
        ...
        output_dir = os.path.join(args.out_dir,'protected')
    '''
    try:
        # ���Խ�������Ϊ JSON �б�����
    ...
    image_list = [
    {"image_name": "27.png","SEED":9955,"prompt":"dog under heavy rain and muddy ground real","strength":0.5,"guidance_scale":7.5}]
    ''',
    ...
    for image_dict in image_list:
        ...
        if args.mode in ['all','protect']:
            if args.protecttype == 'SCIM':
                for SCIMvalue in args.SCIMvalues:
                    eps = args.eps
                    version = f"{SCIMvalue}ScimP01aprBlack{eps}E"
                    ...
                    if args.mode=='protect':
                        adv_image = adv(pipe,image_adv_path, init_image, SCIMvalue,eps)
                    else:
                        ...
                        prompt = imagedict.get("prompt")
                        strength = imagedict.get("strength")
                        guidance_scale = imagedict.get("guidance_scale")
                        SEED = imagedict.get("SEED")
                        image_adv_gened_path = f"{image_adv_path[:image_adv_path.rfind('.')]}_gened.{image_adv_path.split('.')[-1]}"
                        image_nat = infer(pipe,adv_image, image_adv_gened_path, prompt, strength, guidance_scale, SEED)
...
def adv(pipe, image_adv_path,init_image, SCIMvalue,eps):
    if not os.path.exists(image_adv_path):
      with torch.autocast('cuda'):
          ...
          adv_X = pgd(X, 
                      ...
                      step_size=0.2, # Set smaller than eps
    ...
    return image_nat, image_adv, adv_image
